...
test('updateWorkflow', { tag: ["@regression_apiDecimal"] }, async ({ request }) => {
...
module_2: "update"
    ...
    "taskCode": null
        ...
        apiResponse.body().then(body => {
            let expected = '{    "WORKFLOW_API": {        "records": [            {                "primary_key": "",                "data": [                    {                        "status": "success",                        "success": {                            "code": "200",                            "message": "Request Processed Successfully"                        }                    }                ]            }        ],        "responseHandledBy": "MOBILE APP",        "isSuccessRuleValidated": true    }}';
