...
test('deleteFunction', { tag: ["@regression_apiDecimal"] }, async ({ request }) => {
...
module_2: "delete"
    ...
    "code":"rashmi"
        ...
        apiResponse.body().then(body => {
            let expected = '{    "ADMIN_PORTAL_API": {        "records": [            {                "primary_key": "",                "data": [                    {                        "status": "success",                        "success": {                            "code": "200",                            "message": "Resource deleted for - rashmi"                        }                    }                ]            }        ],        "responseHandledBy": "MOBILE APP",        "isSuccessRuleValidated": true    }}';
